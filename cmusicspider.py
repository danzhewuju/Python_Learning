...
def get_hotComments(hot_song_name,hot_song_id):
   ...
   'User-Agent':'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'
    ...
    num=0
    fhandle=open('./song_comments_test.txt','a')  #写入文件
...
def test(params, enSecKey, song_id):
    hot_song_id = song_id
    url = 'http://music.163.com/weapi/v1/resource/comments/R_SO_4_' + hot_song_id + '?csrf_token='  # 歌评url
    header = {
        'User-Agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'
    ...
    comment = json_dict['comments']
    return comment


def getcomment(startpage, song_id, song_name):
    url = 'http://music.163.com/weapi/v1/resource/comments/R_SO_4_' + song_id + '?csrf_token='  # 歌评url
    header = {
        'User-Agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'
    }
    data = {
        'params': spiderlearning1.get_params(startpage),
        'encSecKey': spiderlearning1.get_encSecKey()
    }
    postdata = urllib.parse.urlencode(data).encode('utf-8')
    request = urllib.request.Request(url, headers=header, data=postdata)
    reponse = urllib.request.urlopen(request).read().decode('utf-8')
    ...
    while index < total:
        page = index // 100
        data = {
            'params': spiderlearning1.get_params(page),
            'encSecKey': spiderlearning1.get_encSecKey()
        }
        postdata = urllib.parse.urlencode(data).encode('utf-8')
        request = urllib.request.Request(url, headers=header, data=postdata)
